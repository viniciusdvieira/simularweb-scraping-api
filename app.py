...
def handle_error(e):
    response = jsonify({'success': False, 'error': str(e), 'message': 'Falha ao salvar as informações no banco3.'})
...
async def salve_info():
    try:
        ...
        return jsonify({'success': True, 'message': 'Informações salvas no banco com sucesso.', 'results': results}), 201
    except Exception as e:
        return jsonify({'success': False, 'error': str(e), 'message': 'Falha ao salvar as informações no banco1.'})
...
def get_info_handler(url):
    ...
    if dados_website:
        ...
        return {'success': False, 'error': 'As informações não estão disponíveis no banco de dados2.'}
